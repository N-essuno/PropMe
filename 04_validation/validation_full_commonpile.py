...
def _doc_id_from_metadata(metadata: dict[str, Any], doc_ix: int) -> str:
	for key in ("id", "doc_id", "source", "url", "doc_ix"):
...
def _partial_windows(
	...
	*,
	...
	if not token_ids:
		raise ValueError("Cannot build a query from an empty document.")
	if partial_query_tokens < min_query_tokens:
		raise ValueError("--partial-query-tokens is below --min-query-tokens")

	doc_len = len(token_ids)
	window_len = min(doc_len, partial_query_tokens)
	midpoint = doc_len // 2
	middle_start = max(0, min(midpoint - (window_len // 2), doc_len - window_len))

	return {
		"partial_start": _decode_query_window(
			token_ids,
			tokenizer,
			start=0,
			end=window_len,
		...
		"partial_middle": _decode_query_window(
			token_ids,
			tokenizer,
			start=middle_start,
			end=middle_start + window_len,
			min_query_tokens=min_query_tokens,
		),
		"partial_end": _decode_query_window(
			token_ids,
			tokenizer,
			start=doc_len - window_len,
			end=doc_len,
			min_query_tokens=min_query_tokens,
		),
	}
...
def sample_queries(
	*,
	...
	"""Sample random docs and expand each one into full/start/middle/end queries."""
	if num_samples < 1:
	...
	for doc_ix in candidate_doc_ixs:
		...
		if include_partials:
			try:
				partial_windows = _partial_windows(
					token_ids,
					tokenizer,
		...
		if include_full:
			assert full_window is not None
			exact_start, exact_end, exact_text = full_window
			samples.append(
				_build_query_sample(
	...
	# Print list of sample lengths
	print(
		f"Collected {len(samples)} queries from {source_samples} source documents "
		f"after skipping {skipped} candidates. Sample query token counts: "
		+ ", ".join(str(sample.query_token_count) for sample in samples)
	)
...
def validate_one_result(
	*,
	...
	partial_span_exact_query_match = int(
		is_partial_query
		and any(span.get("text", "") == sample.query_text for span in final_spans)
	)
	partial_query_rescued_by_span_match = int(
		is_partial_query
...
def summarize_validation(rows: list[dict[str, Any]]) -> dict[str, Any]:
	...
	summary = {
		...
		"pass_rate": _mean([row["passed"] for row in rows]),
	...
	for query_kind in ("full", "partial_start", "partial_middle", "partial_end"):
		...
		summary[f"{query_kind}_pass_rate"] = _mean(
			[row["passed"] for row in kind_rows]
		)
		summary[f"{query_kind}_partial_span_exact_query_match_rate"] = _mean(
			[row["partial_span_exact_query_match"] for row in kind_rows]
		)
...
def _format_failed_example_log(
	*,
	sample: QuerySample,
	result: dict[str, Any],
	...
	lines.append(
		"failed_checks: "
		+ json.dumps(validation_row.get("failed_checks", []), ensure_ascii=False)
	)
	...
	lines.append(f"exact_text_match: {validation_row.get('exact_text_match')}")
	lines.append(
...
def write_failed_example_logs(
	trace_rows: list[tuple[QuerySample, dict[str, Any], float]],
	validation_rows: list[dict[str, Any]],
	output_path: str,
) -> dict[str, int]:
	"""Write SimpleTrace-style details for validation examples that failed."""
	failed = [
		...
		if validation_row.get("passed") == 0
	]
	failed_count = sum(1 for row in validation_rows if row.get("passed") == 0)

	os.makedirs(os.path.dirname(output_path) or ".", exist_ok=True)
	with open(output_path, "w", encoding="utf-8") as f:
		if not failed:
			f.write("No validation examples failed.\n")
			return {
				"logged_failed_examples": 0,
				"failed_examples": failed_count,
			}

		for sample, result, runtime_seconds, validation_row in failed:
			f.write(
				_format_failed_example_log(
					sample=sample,
					result=result,
			...
			f.write("\n")
	return {
		"logged_failed_examples": len(failed),
		"failed_examples": failed_count,
	}
...
def build_arg_parser() -> argparse.ArgumentParser:
	...
	parser.add_argument(
		...
		default=128,
		help=(
			"Maximum token length for partial start/middle/end substring checks."
		),
	)
	parser.add_argument(
		"--min-query-tokens",
	...
	parser.add_argument(
		...
		dest="include_full",
		action="store_false",
		help="Disable full source-document/window queries.",
	)
	parser.add_argument(
		"--no-partials",
...
def main() -> None:
	...
	failed_log_stats = write_failed_example_logs(
		trace_rows,
		validation_rows,
		args.failed_log_output,
	)

	summary = summarize_validation(validation_rows)
	summary.update(failed_log_stats)
	summary["config"] = {
		"index_dir": args.index_dir,
	...
	summary["outputs"] = {
		...
		"failed_log_output": args.failed_log_output,
	}
	...
	print(f"Wrote failed-example logs: {args.failed_log_output}")
	rescued_partial_queries = summary.get(
